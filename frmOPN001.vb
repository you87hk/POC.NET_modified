...
Friend Class frmOPN001
	...
	Private Const COAACCCODE As Short = 0
	Private Const COADESC As Short = 1
	Private Const COAOPNBAL As Short = 2
	Private Const COAACCID As Short = 3
	...
	Private Sub Ini_Scr()
		...
		waResult.ReDim(0, -1, COAACCCODE, COAACCID)
		...
		lblDspYear.Text = VB.Left(gsSystemDate, 4)
		...
		Ini_Scr_AfrKey()
		tblDetail.Col = COAOPNBAL
		tblDetail.ScrollBars = TrueDBGrid60.ScrollBarsConstants.dbgVertical
	...
	'UPGRADE_WARNING: Form event frmOPN001.Activate has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
	Private Sub frmOPN001_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
	...
	Private Sub frmOPN001_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
	...
	Private Sub frmOPN001_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
	...
	Private Sub Ini_Caption()
		...
		With tblDetail
			.Columns(COAACCCODE).Caption = Get_Caption(waScrItm, "COAACCCODE")
			.Columns(COADESC).Caption = Get_Caption(waScrItm, "COADESC")
			.Columns(COAOPNBAL).Caption = Get_Caption(waScrItm, "COAOPNBAL")
		End With
...
Ini_Caption_Err: 
	...
	Private Sub frmOPN001_MouseUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles MyBase.MouseUp
	...
	Private Sub frmOPN001_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        ...
        'UPGRADE_NOTE: Object frmOPN001 may not be destroyed until it is garbage collected. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
        Me.Close() ' = Nothing
    End Sub
    ...
    Private Function cmdSave() As Boolean
        ...
        If waResult.get_UpperBound(1) >= 0 Then
            adcmdSave.CommandText = "USP_OPN001"
            adcmdSave.CommandType = ADODB.CommandTypeEnum.adCmdStoredProc
            adcmdSave.Parameters.Refresh()

            For wiCtr = 0 To waResult.get_UpperBound(1)
                If Trim(waResult.get_Value(wiCtr, COAACCCODE)) <> "" Then
                    Call SetSPPara(adcmdSave, 1, lblDspYear.Text)
                    Call SetSPPara(adcmdSave, 2, waResult.get_Value(wiCtr, COAACCID))
                    Call SetSPPara(adcmdSave, 3, waResult.get_Value(wiCtr, COAOPNBAL))
    ...
    Private Function InputValidation() As Boolean
        ...
        With waResult
            For wlCtr = 0 To .get_UpperBound(1)
                If Trim(waResult.get_Value(wlCtr, COAOPNBAL)) <> "" Then
                    wiEmptyGrid = False
...
tblDetail_BeforeRowColChange_Err:
    ...
    Private Sub tblDetail_MouseUpEvent(ByVal eventSender As System.Object, ByVal eventArgs As AxTrueDBGrid60.TrueDBGridEvents_MouseUpEvent) Handles tblDetail.MouseUpEvent
        If eventArgs.Button = 2 Then
            'UPGRADE_ISSUE: Form method frmOPN001.PopupMenu was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
            'PopupMenu(mnuPopUp)
        End If
    End Sub
    ...
    Private Sub Ini_Grid()
        ...
        With tblDetail
            ...
            For wiCtr = COAACCCODE To COAACCID
        ...
        With tblDetail
            Select Case eventArgs.ColIndex
                Case COAOPNBAL
                    ...
                    If Chk_grdBal((.Columns(COAOPNBAL).Text)) = False Then
                        GoTo Tbl_BeforeColUpdate_Err
                    End If

                    .Columns(COAOPNBAL).Text = VB6.Format(.Columns(COAOPNBAL).Text, gsAmtFmt)
            End Select
        End With
    ...
    Private Sub tblDetail_RowColChange(ByVal eventSender As System.Object, ByVal eventArgs As AxTrueDBGrid60.TrueDBGridEvents_RowColChangeEvent) Handles tblDetail.RowColChange
        ...
        With tblDetail
            ...
            If Trim(.Columns(.Col).Text) <> "" Then
                Select Case .Col
                    Case COAOPNBAL
                        Call Chk_grdBal((.Columns(COAOPNBAL).Text))
...
RowColChange_Err:
    ...
    Private Function IsEmptyRow(Optional ByRef inRow As Object = Nothing) As Boolean
        ...
        If IsNothing(inRow) Then
            With tblDetail
                If Trim(.Columns(COAOPNBAL).Text) = "" Then
                    Exit Function
                End If
            End With
        Else
            If waResult.get_UpperBound(1) >= 0 Then
                If Trim(waResult.get_Value(inRow, COAACCCODE)) = "" And Trim(waResult.get_Value(inRow, COADESC)) = "" And Trim(waResult.get_Value(inRow, COAOPNBAL)) = "" Then
                    Exit Function
                End If
            End If
        End If
    ...
    Private Function Chk_GrdRow(ByVal LastRow As Integer) As Boolean
        ...
        With tblDetail
			...
			If Chk_grdBal(waResult.get_Value(LastRow, COAOPNBAL)) = False Then
				.Col = COAOPNBAL
				.Row = LastRow
				Exit Function
			End If
...
Chk_GrdRow_Err: 
	...
	Private Function LoadRecord() As Boolean
		...
		If CDbl(gsLangID) = 1 Then
			wsSQL = "SELECT COAAccID, COAAccCode, COACDESC COADESC, COAOPNBAL "
			wsSQL = wsSQL & "FROM  MstCOA "
			wsSQL = wsSQL & "WHERE COAStatus = '1' "
			wsSQL = wsSQL & "ORDER BY COAACCCODE "
		Else
			wsSQL = "SELECT COAAccID, COAAccCode, COADESC, COAOPNBAL "
			wsSQL = wsSQL & "FROM  MstCOA "
			wsSQL = wsSQL & "WHERE COAStatus = '1' "
			wsSQL = wsSQL & "ORDER BY COAACCCODE "
		End If
		...
		rsRcd.MoveFirst()
		With waResult
			.ReDim(0, -1, COAACCCODE, COAACCID)
			Do While Not rsRcd.EOF
				wiCtr = wiCtr + 1
				.AppendRows()
				'UPGRADE_WARNING: Couldn't resolve default property of object ReadRs(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				waResult.get_Value(.get_UpperBound(1), COAACCCODE) = ReadRs(rsRcd, "COAACCCODE")
				'UPGRADE_WARNING: Couldn't resolve default property of object ReadRs(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				waResult.get_Value(.get_UpperBound(1), COADESC) = ReadRs(rsRcd, "COADESC")
				'UPGRADE_WARNING: Couldn't resolve default property of object ReadRs(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				waResult.get_Value(.get_UpperBound(1), COAOPNBAL) = ReadRs(rsRcd, "COAOPNBAL")
				'UPGRADE_WARNING: Couldn't resolve default property of object ReadRs(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				waResult.get_Value(.get_UpperBound(1), COAACCID) = ReadRs(rsRcd, "COAACCID")
				rsRcd.MoveNext()
			Loop 
