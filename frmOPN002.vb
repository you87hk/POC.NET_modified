...
Friend Class frmOPN002
	...
	Private Const ITMQTY As Short = 3
	...
	Private Const wsKeyType As String = "ICInventory"
	...
	'UPGRADE_WARNING: Form event frmOPN002.Activate has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
	Private Sub frmOPN002_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
	...
	Private Sub frmOPN002_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
	...
	Private Sub frmOPN002_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
...
Ini_Caption_Err: 
	...
	Private Sub frmOPN002_MouseUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles MyBase.MouseUp
	...
	Private Sub frmOPN002_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        ...
        'UPGRADE_NOTE: Object frmOPN002 may not be destroyed until it is garbage collected. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
        Me.Close() ' = Nothing
    End Sub
        ...
        If waResult.get_UpperBound(1) >= 0 Then
            ...
            For wiCtr = 0 To waResult.get_UpperBound(1)
                If Trim(waResult.get_Value(wiCtr, ITMCODE)) <> "" Then
                    ...
                    Call SetSPPara(adcmdSave, 2, waResult.get_Value(wiCtr, ITMID))
                    Call SetSPPara(adcmdSave, 3, waResult.get_Value(wiCtr, ITMWHSCODE))
                    Call SetSPPara(adcmdSave, 4, waResult.get_Value(wiCtr, ITMQTY))
                    Call SetSPPara(adcmdSave, 5, lblDspYear.Text & "/01/01")
                    Call SetSPPara(adcmdSave, 6, lblDspYear.Text & "/01/01")
                    Call SetSPPara(adcmdSave, 7, "9999/99/99")
                    Call SetSPPara(adcmdSave, 8, lblDspYear.Text & "01")
    ...
    Private Function InputValidation() As Boolean
        ...
        With waResult
            For wlCtr = 0 To .get_UpperBound(1)
                If Trim(waResult.get_Value(wlCtr, ITMQTY)) <> "" Then
                    wiEmptyGrid = False
...
tblDetail_BeforeRowColChange_Err:
    ...
    Private Sub tblDetail_MouseUpEvent(ByVal eventSender As System.Object, ByVal eventArgs As AxTrueDBGrid60.TrueDBGridEvents_MouseUpEvent) Handles tblDetail.MouseUpEvent
        If eventArgs.Button = 2 Then
            'UPGRADE_ISSUE: Form method frmOPN002.PopupMenu was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
            'PopupMenu(mnuPopUp)
        End If
    End Sub
        ...
        With tblDetail
            Select Case eventArgs.ColIndex
                Case ITMQTY
                    ...
                    .Columns(ITMQTY).Text = VB6.Format(.Columns(ITMQTY).Text, gsAmtFmt)
            End Select
        End With
...
RowColChange_Err:
    ...
    Private Function IsEmptyRow(Optional ByRef inRow As Object = Nothing) As Boolean
        ...
        If IsNothing(inRow) Then
            With tblDetail
                If Trim(.Columns(ITMQTY).Text) = "" Then
                    Exit Function
                End If
            End With
        Else
            If waResult.get_UpperBound(1) >= 0 Then
                If Trim(waResult.get_Value(inRow, ITMCODE)) = "" And Trim(waResult.get_Value(inRow, ITMQTY)) = "" Then
                    Exit Function
                End If
            End If
        End If
    ...
    Private Function Chk_GrdRow(ByVal LastRow As Integer) As Boolean
        ...
        With tblDetail
			...
			If Chk_grdBal(waResult.get_Value(LastRow, ITMQTY)) = False Then
				.Col = ITMQTY
				.Row = LastRow
				Exit Function
			End If
	...
	Private Function LoadRecord() As Boolean
		...
		With waResult
			...
			Do While Not rsRcd.EOF
				...
				'UPGRADE_WARNING: Couldn't resolve default property of object ReadRs(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				waResult.get_Value(.get_UpperBound(1), ITMWHSCODE) = ReadRs(rsRcd, "WHSCODE")
				'UPGRADE_WARNING: Couldn't resolve default property of object ReadRs(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
				waResult.get_Value(.get_UpperBound(1), ITMID) = ReadRs(rsRcd, "ITMID")
				rsRcd.MoveNext()
			Loop 
		...
		If waResult.get_UpperBound(1) >= 0 Then
			For wiCtr = 0 To waResult.get_UpperBound(1)
				If Trim(waResult.get_Value(wiCtr, ITMCODE)) <> "" Then
					...
					wsSQL = wsSQL & "WHERE icItemID = " & waResult.get_Value(wiCtr, ITMID) & " "
					wsSQL = wsSQL & "AND icWhsCode = '" & Set_Quote(waResult.get_Value(wiCtr, ITMWHSCODE)) & "' "
					wsSQL = wsSQL & "AND icSrcCode = 'OPN' "
					wsSQL = wsSQL & "AND icStatus = '4' "
					rsRcd.Open(wsSQL, cnCon, ADODB.CursorTypeEnum.adOpenStatic, ADODB.LockTypeEnum.adLockOptimistic)
					
					If rsRcd.RecordCount <= 0 Then
						waResult.get_Value(wiCtr, ITMQTY) = VB6.Format(0, gsQtyFmt)
						rsRcd.Close()
					Else
						'UPGRADE_WARNING: Couldn't resolve default property of object ReadRs(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
						waResult.get_Value(wiCtr, ITMQTY) = VB6.Format(ReadRs(rsRcd, "icTrnQty"), gsQtyFmt)
						rsRcd.Close()
					End If
