...
Friend Class frmSAM001
	...
	Private Const PUBLISHER As Short = 7
...
EventExitSub: 
	...
	Private Sub Ini_Scr_AfrKey()
		...
		If LoadRecord() = False Then
			...
			cboCusCode.Focus()
			cboMLCode.Text = Get_CompanyFlag("CmpSamMLCode")
		Else
			wiAction = CorRec
	...
	Private Sub cboWhsCodeFr_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles cboWhsCodeFr.KeyPress
		...
		If KeyAscii = System.Windows.Forms.Keys.Return Then
            ...
            lblDspWhsDesc.Text = Get_TableInfo("MstWarehouse", "WHSCODE = '" & Set_Quote(cboWhsCodeFr.Text) & "'", "WHSDESC")
            cboMLCode.Focus()
...
EventExitSub:
    ...
    'UPGRADE_WARNING: Form event frmSAM001.Activate has a new behavior. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6BA9B8D2-2A32-4B6E-8D36-44949974A5B4"'
    Private Sub frmSAM001_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
    ...
    Private Sub frmSAM001_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub frmSAM001_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
    ...
    Private Function LoadRecord() As Boolean
        ...
        wsSQL = "SELECT SJHDDOCID, SJHDDOCNO, SJHDDOCDATE, SJDTDOCLINE, SJHDSTAFFID, SJHDMLCODE,SJHDREMARK,SJHDREMARK2,SJHDREMARK3, "
        wsSQL = wsSQL & "SJDTITEMID, ITMCODE, ITMITMTYPECODE, SJDTWHSCODE, SJDTLOTNO, ITMBARCODE, "
        ...
        wlCusID = To_Value(ReadRs(rsInvoice, "SJHDCUSID"))

        cboCusCode.Text = Get_TableInfo("MstCustomer", "CUSID = " & CStr(wlCusID), "CusCode")
        lblDspCusDesc.Text = Get_TableInfo("MstCustomer", "CUSID = " & CStr(wlCusID), "CusName")
    ...
    Private Sub Ini_Caption()
        ...
        optInOut(0).Text = Get_Caption(waScrItm, "STKOUT")
        optInOut(1).Text = Get_Caption(waScrItm, "STKIN")
        ...
        wsActNam(1) = Get_Caption(waScrItm, "SAMADD")
        wsActNam(2) = Get_Caption(waScrItm, "SAMEDIT")
        wsActNam(3) = Get_Caption(waScrItm, "SAMDELETE")

        wgsTitle = Get_Caption(waScrItm, "TITLE")
        wgsTRTitle = Get_Caption(waScrItm, "SMTITLE")
...
Ini_Caption_Err:
    ...
    Private Sub frmSAM001_MouseUp(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.MouseEventArgs) Handles MyBase.MouseUp
    ...
    'UPGRADE_WARNING: Event frmSAM001.Resize may fire when form is initialized. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="88B12AE1-6DE0-48A0-86F1-60C0686C026A"'
    Private Sub frmSAM001_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
    ...
    Private Sub frmSAM001_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        ...
        'UPGRADE_NOTE: Object frmSAM001 may not be destroyed until it is garbage collected. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
        Me.Close() ' = Nothing
        ...
        If waResult.get_UpperBound(1) >= 0 Then
            ...
            For wiCtr = 0 To waResult.get_UpperBound(1)
                If Trim(waResult.get_Value(wiCtr, ITMCODE)) <> "" Then
                    ...
                    Call SetSPPara(adcmdSave, 7, waResult.get_Value(wiCtr, LOTNO))
                    Call SetSPPara(adcmdSave, 8, waResult.get_Value(wiCtr, PRICE))
...
RowColChange_Err:
    ...
    Private Function IsEmptyRow(Optional ByRef inRow As Object = Nothing) As Boolean
        ...
        Else
            If waResult.get_UpperBound(1) >= 0 Then
                If Trim(waResult.get_Value(inRow, ITMTYPE)) = "" And Trim(waResult.get_Value(inRow, ITMCODE)) = "" And Trim(waResult.get_Value(inRow, ITMNAME)) = "" And Trim(waResult.get_Value(inRow, PUBLISHER)) = "" And Trim(waResult.get_Value(inRow, QTY)) = "" And Trim(waResult.get_Value(inRow, ITMID)) = "" Then
                    Exit Function
                End If
            End If
        End If
...
cmdDelete_Err:
    ...
    Private Sub OpenPromptForm()
        ...
        'UPGRADE_ISSUE: Form property frmSAM001.MousePointer does not support custom mousepointers. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="45116EAB-7060-405E-8ABE-9DBB40DC2E86"'
    ...
    Private Sub tblDetail_MouseUpEvent(ByVal eventSender As System.Object, ByVal eventArgs As AxTrueDBGrid60.TrueDBGridEvents_MouseUpEvent) Handles tblDetail.MouseUpEvent
        If eventArgs.Button = 2 Then
            'UPGRADE_ISSUE: Form method frmSAM001.PopupMenu was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
            'PopupMenu(mnuPopUp)
        End If
    ...
    Private Function GetInOut() As String
        ...
        For iCounter = 0 To 5
            If optInOut(iCounter).Checked = True Then
                Exit For
            End If
        Next
    ...
    Private Sub cboCusCode_DropDown(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cboCusCode.DropDown
        ...
        wsSQL = "SELECT CUSCODE, CUSNAME FROM MstCustomer WHERE CusCode LIKE '%" & IIf(cboCusCode.SelectionLength > 0, "", Set_Quote(cboCusCode.Text)) & "%' "
        wsSQL = wsSQL & "AND CusStatus = '1' "
        wsSQL = wsSQL & "AND CusInactive = 'N' "
        wsSQL = wsSQL & "ORDER BY CusCode "
        Call Ini_Combo(2, wsSQL, (VB6.PixelsToTwipsX(cboCusCode.Left)), VB6.PixelsToTwipsY(cboCusCode.Top) + VB6.PixelsToTwipsY(cboCusCode.Height), tblCommon, wsFormID, "TBLCUSCOD", VB6.PixelsToTwipsX(Me.Width), VB6.PixelsToTwipsY(Me.Height))
    ...
    Private Function chk_cboCusCode() As Boolean
        ...
        If Chk_CusCode(cboCusCode.Text, wlCusID, wsDesc, "", "") = False Then
            gsMsg = "沒有此客戶!"
            MsgBox(gsMsg, MsgBoxStyle.OKOnly, gsTitle)
            cboCusCode.Focus()
            lblDspCusDesc.Text = ""
